...
```{r residual}
predict = predict(m1, newdata = data.frame(at_bats = c(5579)))
...
```{r runs-vs-others}
m4 <- lm(runs ~ strikeout, data = mlb11)
mlb11 %>% 
  ggplot(aes(strikeout, runs)) +
  geom_point() +
  stat_smooth(method = "lm", se = FALSE)
summary(m4)
